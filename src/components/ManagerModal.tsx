...
import { BarChart3, Eye, EyeOff, KeyRound, LockKeyhole, Mail, RefreshCw, Save, ShieldCheck, X } from 'lucide-react';
...
const PasswordField = ({ label, value, onChange, visible, onToggle, onEnter, autoFocus }: { label: string; value: string; onChange: (value: string) => void; visible: boolean; onToggle: () => void; onEnter?: () => void; autoFocus?: boolean }) => <label className="block text-xs font-semibold">{label}<span className="relative mt-1.5 block"><input value={value} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && onEnter?.()} type={visible ? 'text' : 'password'} className="w-full rounded-lg border border-slate-700 bg-slate-950 p-3 pr-10 text-sm" autoFocus={autoFocus} /><button type="button" onClick={onToggle} aria-label={visible ? 'Ocultar clave' : 'Mostrar clave'} className="absolute inset-y-0 right-0 px-3 text-slate-400 hover:text-white">{visible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span></label>;
